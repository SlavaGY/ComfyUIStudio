...
| 5 | Cleanup / packaging (старые `sys.path` hacks, compat-импорты, дубли `shared_*`, тесты, оба PyInstaller-профиля) | ◻ Не начат | 3 и 4 (оба должны быть завершены — иначе cleanup придётся делать дважды) | средний — легко случайно удалить что-то ещё используемое; страхуется чек-листом этапа 0 |
...
Этапы 3 и 4 выполнены — оба зависели только от этапа 2 и не пересекались
друг с другом. Следующий на очереди — этап 5 (cleanup): убрать
`sys.path`-хак в корневом `main.py`, дубли `shared_*` в `prompt_builder/`
и `promptvault/`, перевести тесты PromptVault на `comfyui_studio.promptvault`
(бо́льшая часть, впрочем, уже сделана — см. этап 2) и слить
`tools/promptvault/pyproject.toml` с корневым.
