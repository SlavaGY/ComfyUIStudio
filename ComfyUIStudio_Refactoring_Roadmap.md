...
- **очередь задач, история генераций** — прямое расширение `get_queue()`/`get_history()` из этапа 6, не затронуто ограничением ниже;
- **какие модели используются, загрузка/выгрузка моделей** — через `get_system_stats()` (VRAM per-model) и `get_object_info()`, не затронуто;
- **состояние custom nodes** — через `get_object_info()`, не затронуто;
- **текущий workflow, выполняемая нода, прогресс** и **ошибки конкретного workflow** — планировались через WS `executing`/`progress`/`executed`/`execution_error` из этапа 7. ⚠️ После фикса регрессии в этапе 7 выяснилось (сверено с исходником ComfyUI, см. подраздел "Окончательный ответ" в тексте этапа 7): эти события ComfyUI шлёт НЕ broadcast'ом, а только тому WS-клиенту, чей `clientId` совпадает с `clientId`, которым конкретное задание было поставлено в очередь через `/prompt`. В этом лаунчере задания ставит встроенный браузер под своим, не нашим `clientId` — поэтому `ComfyWebSocketClient` в его нынешнем виде НИКОГДА не получит эти события, вне зависимости от того, что мы туда подставим. Чтобы этот пункт заработал, сначала нужна отдельная, не сделанная пока задача — прочитать реальный `clientId` встроенного браузера (например, через `QWebEngineView.page().runJavaScript(...)`, читая ключ, под которым ComfyUI хранит его в `localStorage` страницы) и передавать именно его в `ComfyWebSocketClient` вместо собственного `uuid4`. Не проверено вживую (нет доступа к реальному ComfyUI/точному имени ключа в конкретной версии фронтенда) — если этот путь понадобится, начинать стоит с него, до попыток разбирать сами `executing`/`progress`/`executed` события.
...
Этапы 0–7 выполнены. Следующий на очереди — этап 8 (новая
функциональность поверх этапов 6 и 7 вместе). Из трёх пунктов, изначально оставленных
...
песочнице нет Windows/Qt-окружения для этого. У этапов 6 и 7 свой
открытый пункт с тем же корнем причины (нет живого ComfyUI в
песочнице) — точная схема ответа `/system_stats` (этап 6) и сообщения
`"progress"` (этап 7) не сверялась вживую, см. отступления "По факту
реализации" в тексте обоих этапов.
