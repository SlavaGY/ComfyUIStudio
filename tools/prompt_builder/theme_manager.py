...
import prompt_builder.shared_theme as shared_theme
...
def resource_base() -> Path:
    ...
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        ...
        # инструмента в tools/prompt_builder/... под _MEIPASS -- так же,
        # как они лежат рядом с исходниками -- специально НЕ в корень
        # _MEIPASS, чтобы не перезаписать одноимённую themes/ папку
        # ComfyUI Launcher при сборке (см. build/ComfyUIStudio.spec).
        return Path(sys._MEIPASS) / "tools" / "prompt_builder"
