...
настраивает извне — из единого дерева настроек ComfyUI Studio (см.
comfyui_studio/launcher/ui/settings/prompt_builder_page.py; раньше
указывалась изнутри самого редактора через меню "Файл -> Указать папку
с файлами LoRA...", которого больше нет, см. main.py) — хранится в
QSettings, общая на весь редактор (и для "Привязанная LoRA" в
CharactersTab, и для LoraTableEditor в PromptBuilderTab). Список
пересканирует папку заново при каждом разворачивании — если файлы
добавили/удалили или сама папка была изменена в настройках, пока
редактор был открыт, ничего перезапускать не нужно.
"""
