...
def main(argv=None):
    ...
    # НОВОЕ (живой отчёт: "из второй генерации из двух картинок дошла
    # только одна" -- при этом сама генерация ОДНА (`batch_size` внутри
    # ОДНОГО queue_prompt(), см. imagine/backend/main.py -- не два
    # отдельных прода), значит обе картинки были одновременно доступны
    # в одном ответе статуса, и упало именно СКАЧИВАНИЕ второй по
    # счёту): timeout_keep_alive по умолчанию у uvicorn -- 5 секунд.
    # GeneratedImageSaver.kt на телефоне держит один статический
    # OkHttpClient (со своим пулом соединений, по умолчанию живущим 5
    # МИНУТ) и качает картинки этой генерации ПОСЛЕДОВАТЕЛЬНО -- пока
    # первая пишется в MediaStore (реальное время на диске), соединение
    # с Remote простаивает; если это заняло больше 5с, Remote (со
    # стороны uvicorn) уже закрыл его как неактивное, а OkHttp может
    # попытаться переиспользовать для второго запроса уже мёртвый сокет
    # -- та же гонка, что и была найдена и исправлена на хопе Remote→
    # Imagine (см. imagine_proxy.py, max_keepalive_connections=0), только
    # здесь она на хопе телефон→Remote, и здесь клиент (OkHttp на
    # телефоне) не наш код, чтобы также просто отключить у НЕГО keep-
    # alive -- поэтому чинится с другой стороны: увеличиваем таймаут
    # простоя у самого Remote, чтобы он не закрывал соединение раньше,
    # чем OkHttp на телефоне сочтёт его протухшим.
    uvicorn.run(app, host=args.host, port=args.port, log_level="info", timeout_keep_alive=75)
