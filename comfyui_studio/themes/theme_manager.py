...
# launcher.log благодаря стандартному наследованию обработчиков по
# имени "comfyui_launcher.*".
log = logging.getLogger("comfyui_launcher.themes")

# см. подробный комментарий у аналогичного кода в
# comfyui_studio/launcher/core/logging_setup.py (resource_path) —
# Path(__file__).resolve().parent сам по себе не находит файлы внутри
# сборки PyInstaller, нужен sys._MEIPASS.
if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
    # После переноса под общее пространство имён (этап 2 дорожной
    # карты) темы лежат в _MEIPASS/comfyui_studio/themes. Сам .spec
    # ещё предстоит обновить под новую раскладку (этап 5).
