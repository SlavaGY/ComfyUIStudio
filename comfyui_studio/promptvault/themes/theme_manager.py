...
# сборки PyInstaller, нужен sys._MEIPASS.
if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
    THEMES_DIR = Path(sys._MEIPASS) / "comfyui_studio" / "promptvault" / "themes"
else:
...
class ThemeManager(QObject):
    ...
    ComfyUI Launcher или PromptConfigEditor, пока это приложение уже
    открыто, она применяется сразу (сигнал theme_changed_externally).
    """
    ...
    def current_theme(self):
        ...
        подхватывается тема, выбранная в ComfyUI Launcher или
        PromptConfigEditor; если её нет, откатываемся на собственные
        QSettings, а затем на тему по умолчанию."""
    ...
    def apply_theme(self, theme_name, app=None):
        ...
        # Общая тема комплекта — чтобы ComfyUI Launcher и
