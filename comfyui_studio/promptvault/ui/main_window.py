...
from comfyui_studio.promptvault.ui.image_viewer import ImageViewer
from comfyui_studio.promptvault.ui.info_panel import InfoPanel
...
from comfyui_studio.promptvault.ui.sort_popup import SortPopup
from comfyui_studio.promptvault.ui.statistics_window import StatisticsWindow
from comfyui_studio.promptvault.ui.thumbnail_panel import ThumbnailPanel
from comfyui_studio.promptvault.ui.toolbar import Toolbar
from comfyui_studio.promptvault.utils import open_file_externally, reveal_in_file_manager
...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event) -> None:
        ...
        if self._pending_restart:
            ...
            # было запущено через `python -m comfyui_studio.promptvault.main`:
            # -m подставляет в sys.argv[0] путь к уже РАЗРЕШЁННОМУ файлу
            # модуля (например, C:\...\comfyui_studio\promptvault\main.py),
            # и повторный запуск этого пути КАК ОБЫЧНОГО СКРИПТА (без -m)
            # кладёт в sys.path директорию promptvault/, а не корень
            # проекта — импорт `from comfyui_studio.promptvault.core...`
            # в самом же main.py тут же падает с ModuleNotFoundError: No
            # module named 'comfyui_studio'.
            #
            # Поэтому пересобираем вызов явно как
            # `-m comfyui_studio.promptvault.main` (единственный
            # документированный способ запуска — см. CONTRIBUTING.md)
            # вместо повторного использования sys.argv[0] как есть.
            # sys.argv[1:] (дополнительные CLI-аргументы, если они
            # когда-нибудь появятся) сохраняются — они не зависят от
            # того, использовался -m или нет.
            os.execv(python, [python, "-m", "comfyui_studio.promptvault.main"] + sys.argv[1:])
